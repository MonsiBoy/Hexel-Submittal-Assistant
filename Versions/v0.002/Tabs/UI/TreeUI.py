...
class BookmarkTree(QTreeWidget):
    ...
    def _wrap_drop_event(self, original_drop_event):
        def wrapper(event):
            ...
            if not selected_items or target_item in selected_items:
                # Prevent dropping onto self or empty drop
                return original_drop_event(event)

            for item in selected_items:
                # Prevent re-parenting to descendant
                ancestor = target_item
                while ancestor:
                    if ancestor == item:
                        return  # Do not allow drop onto own child
