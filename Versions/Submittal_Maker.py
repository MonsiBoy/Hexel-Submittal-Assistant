...
def generate_pdf(data, desktop_path):
    ...
    est_width2 = pdfmetrics.stringWidth('Page 999 ', 'Times-Bold', 12)
    est_width3 = pdfmetrics.stringWidth('Page 999 of 999 ', 'Times-Bold', 12)
    ...
    for item, value in data['item'].items():
        ...
        c.drawString(left_margin+est_width+100, y_curr, f'Page {list(data['item'][item].items())[0][1]}')
        c.drawString((left_margin+est_width+100)+est_width2+7, y_curr, f'of {data['total pages']}')
        c.drawString((left_margin+est_width+100)+(est_width2+7)+(est_width3-50), y_curr, '~')
        c.drawString((left_margin+est_width+100)+(est_width2+7)+(est_width3-50)+14, y_curr, f'Page {list(data['item'][item].items())[1][1]}')
        c.drawString((left_margin+est_width+100)+(est_width2+7)+(est_width3-50)+14+(est_width2+7), y_curr, f'of {data['total pages']}')
        y_curr -= 4.5*mm
    y_curr -= 30.5*mm
